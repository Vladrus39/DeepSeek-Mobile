...
use crate::workspace_layout::PROJECT_WORKSPACE_DIR_NAME;
...
{host_step}\n\
   - Windows one-click: Setup-DeepSeek-PC-Host.ps1 (firewall UAC once + autostart task)\n\
   - Windows manual: start-deepseek-pc-host.ps1\n\
   - Linux:   start-deepseek-pc-host.sh\n\
   - macOS:   start-deepseek-pc-host.command\n\
3. Projects live in the \"{project_workspace}\" folder next to this bundle.\n\
4. Keep the terminal window open while the Android app is connected.\n\n\
...
Security note: this bundle contains a pairing token. Do not share it with untrusted people.\n",
            ...
            project_workspace = PROJECT_WORKSPACE_DIR_NAME,
        )
    }
    ...
    pub fn bundle_files(
        ...
        files.push(PcPairingBundleFile {
            relative_path: "Setup-DeepSeek-PC-Host.ps1".to_string(),
            content: self.windows_one_click_setup_script(),
            executable: false,
        });
        files.push(PcPairingBundleFile {
            relative_path: "Setup-DeepSeek-PC-Host.cmd".to_string(),
            content: self.windows_one_click_setup_cmd(),
            executable: false,
        });
...
if ($env:DEEPSEEK_PC_HOST_WORKSPACE -eq "." -or $env:DEEPSEEK_PC_HOST_WORKSPACE -eq "{project_workspace}") {{
    $env:DEEPSEEK_PC_HOST_WORKSPACE = Join-Path $ScriptDir "{project_workspace}"
}}
New-Item -ItemType Directory -Force -Path $env:DEEPSEEK_PC_HOST_WORKSPACE | Out-Null
$LocalCandidates = @(
...
"#,
            ...
            project_workspace = escape_powershell(PROJECT_WORKSPACE_DIR_NAME),
        )
    }

    pub fn windows_one_click_setup_cmd(&self) -> String {
        "@echo off\r\n\
cd /d \"%~dp0\"\r\n\
powershell -NoProfile -ExecutionPolicy Bypass -File \"%~dp0Setup-DeepSeek-PC-Host.ps1\" %*\r\n\
if errorlevel 1 pause\r\n"
            .to_string()
    }

    pub fn windows_one_click_setup_script(&self) -> String {
        let project_workspace = escape_powershell(PROJECT_WORKSPACE_DIR_NAME);
        format!(
            r#"param([switch]$Elevated)
$ErrorActionPreference = "Stop"
$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
Set-Location $ScriptDir

$EnvFile = Join-Path $ScriptDir "deepseek-pc-host.env"
if (-not (Test-Path $EnvFile)) {{
    Write-Error "Missing deepseek-pc-host.env. Unzip the pairing bundle first."
}}
...
New-Item -ItemType Directory -Force -Path $env:DEEPSEEK_PC_HOST_WORKSPACE | Out-Null
Set-Content -Path $EnvFile -Value (Get-Content $EnvFile | ForEach-Object {{
    if ($_ -match '^DEEPSEEK_PC_HOST_WORKSPACE=') {{
        "DEEPSEEK_PC_HOST_WORKSPACE=$($env:DEEPSEEK_PC_HOST_WORKSPACE)"
    }} else {{ $_ }}
}})
...
function Ensure-FirewallRule {{
    param([string]$DisplayName, [string]$Direction, [string]$Protocol, [int[]]$LocalPort)
    $existing = Get-NetFirewallRule -DisplayName $DisplayName -ErrorAction SilentlyContinue
    if ($existing) {{ Write-Host "[OK] $DisplayName"; return }}
    New-NetFirewallRule -DisplayName $DisplayName -Direction $Direction -Action Allow -Protocol $Protocol -LocalPort $LocalPort -Profile Private | Out-Null
    Write-Host "[+] $DisplayName"
}}

if (-not $Elevated -and -not (Test-IsAdmin)) {{
    Write-Host "Requesting Administrator approval for Windows Firewall (one-time)..."
...
$tcpPort = 8787
if ($env:DEEPSEEK_PC_HOST_BIND -match ':(\d+)$') {{ $tcpPort = [int]$Matches[1] }}
...
if (-not $BinaryPath) {{
    Write-Error "deepseek-pc-host binary not found next to this bundle."
}}

$TaskName = "DeepSeekPcHost"
...
Write-Host ""
Write-Host "DeepSeek PC Host is installed." -ForegroundColor Green
Write-Host "Workspace: $($env:DEEPSEEK_PC_HOST_WORKSPACE)"
Write-Host "Bind: $($env:DEEPSEEK_PC_HOST_BIND)"
...
if [ "$DEEPSEEK_PC_HOST_WORKSPACE" = "." ] || [ "$DEEPSEEK_PC_HOST_WORKSPACE" = "{project_workspace}" ]; then
  DEEPSEEK_PC_HOST_WORKSPACE="$SCRIPT_DIR/{project_workspace}"
  export DEEPSEEK_PC_HOST_WORKSPACE
fi
mkdir -p "$DEEPSEEK_PC_HOST_WORKSPACE"
...
mod tests {
    ...
    fn bundle_file_manifest_contains_expected_files() {
        ...
        assert!(files
            .iter()
            .any(|file| file.relative_path == "Setup-DeepSeek-PC-Host.ps1"));
    }
