# E2E: snapshot_list on app sandbox store (no LLM, no restore).
param(
...
Write-Host "=== snapshots list probe ===" -ForegroundColor Cyan
Invoke-Adb @("shell", "am", "start", "-n", "$pkg/dev.dioxus.main.MainActivity") | Out-Null
Start-Sleep 3
Invoke-Adb @("shell", "run-as", $pkg, "rm", "-f", "$data/.snapshots_list_probe_result") | Out-Null
Invoke-Adb @("shell", "run-as", $pkg, "touch", "$data/.snapshots_list_probe_requested") | Out-Null

$deadline = (Get-Date).AddSeconds(45)
$result = $null
while ((Get-Date) -lt $deadline) {
    Start-Sleep 2
    $raw = Invoke-Adb @("shell", "run-as", $pkg, "cat", "$data/.snapshots_list_probe_result")
