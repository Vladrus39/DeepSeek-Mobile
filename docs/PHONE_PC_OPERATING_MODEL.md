...
2. The bundle contains `pairing.json`, `deepseek-pc-host.env`, `README.txt`, manual launch scripts, and **Windows one-click setup** (`Setup-DeepSeek-PC-Host.cmd` / `.ps1`).
3. User unzips on the PC and double-clicks **`Setup-DeepSeek-PC-Host.cmd`** (or runs a manual launcher).
4. One-click setup (Windows): loads env, creates **`Project workspace`** next to the bundle, prompts once for Administrator approval to add **Private-network firewall** rules (gateway TCP port + mDNS UDP 5353), registers a **logon scheduled task**, and starts `deepseek-pc-host`.
...
When the agent needs a path outside **Project workspace**, the user approves the tool in chat; the phone calls **`grant_trusted_path`** on the PC gateway for that session.
...
Target v1 packaging (largely shipped):

1. Include the matching `deepseek-pc-host` binary in the PC pairing/release package when possible.
2. **`Setup-DeepSeek-PC-Host`** in the ZIP handles firewall + autostart without a separate repo installer.
3. Optional signed standalone installer remains a future enhancement for users who never use the phone export path.
...
This is a **PC gateway** (HTTP/WebSocket + bearer token), **not** a VPN or TLS tunnel by default on LAN.

- Pairing token is generated by the phone and stored in the pairing bundle/ZIP — treat the ZIP as a secret.
- Default LAN transport is **plain HTTP** (`LocalNetworkHttp`); use HTTPS modes for internet-facing endpoints.
- Anyone on the **same LAN** who learns IP, port, and token can call the gateway; Windows setup opens firewall on **Private** profile only.
- PC Host serves the **`Project workspace`** root from pairing; path traversal is rejected at the host/core boundary.
- Additional directories require `DEEPSEEK_PC_HOST_TRUSTED_PATHS` in the bundle and/or **chat approval grant** (`grant_trusted_path`) for one-off paths.
...
- Public remote access must require explicit configuration and HTTPS (`TunnelHttps` / `InternetHttps`).
...
- pairing ZIP one-click setup shipped; optional standalone signed installer still open;
