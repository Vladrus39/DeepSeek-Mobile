...
3. ~~Build durable background tasks, runtime API, then MCP/plugins/skills.~~ Background task infrastructure ✅ (2026-05-25): PC host can spawn/stop/list background tasks via `run_task`/`stop_task`/`list_tasks` gateway RPC; tool routing wired for `detect_tasks`, `task_run`, `task_stop`, `task_list`. Durable task records with queue lifecycle and JSON persistence ✅ (2026-05-25): `DurableTaskManager` in core with full CRUD, status transitions, and 16 tests. Mobile task manager UI ✅ (2026-05-25): `TasksUiState` + `tasks_panel` with filter chips, task cards, cancel/prune buttons. MCP/skills ✅ (2026-05-25): `SkillRegistry`/`SkillManifest` with frontmatter parser; `McpClientRegistry`/`McpServerConfig`/`McpTransport` (stdio + http-sse) with JSON persistence; Skills UI panel; MCP server status UI panel; both wired into drawer/navbar/cockpit.
